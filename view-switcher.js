(() => {
  ...
  nav.innerHTML = `<a href="${new URL('desk/', base)}" ${isDesk ? 'aria-current="page"' : ''}>Desk View</a><a href="${new URL('index.html?view=academic', base)}" ${!isDesk ? 'aria-current="page"' : ''}>Academic View</a>`;
  const [desk, academic] = nav.children;
  function update() {
    const key = context();
    desk.href = new URL('desk/' + (isDesk ? location.hash : key ? '#' + key : ''), base);
    academic.href = new URL('index.html?view=academic' + (key ? '#' + key : ''), base);
  }
